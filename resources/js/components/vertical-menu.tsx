...
import VerticalMenuNode, { VerticalMenuContext } from '@/components/vertical-menu-node';
...
const activeGroupKeys = (nodes: MenuNode[], path: string, trail: string[] = []): string[] => {
    const keys: string[] = [];
...
continue;
        ...
        const childKeys = activeGroupKeys(n.children, path, [...trail, n.label]);
...
export default function VerticalMenu() {
    ...
    const { props } = usePage<{ name: string; tenant: { id: number } | null }>();
    const appName = props.name;
    const activePage = tenants.find(page => page.id === props.tenant?.id) ?? null;
    ...
    const toggleGroup = useCallback((key: string) => {
        setOpenGroups(prev => (prev.includes(key) ? ancestorsOf(key) : [...ancestorsOf(key), key]));
    }, []);
    ...
    const vmenu = useMemo(() => ({ openGroups, toggleGroup }), [openGroups, toggleGroup]);

    return (
        <VerticalMenuContext value={vmenu}>
            {/* menu shadow */}
            <div className={cn('menu-shadow', !isMobileMenuOpen && 'hidden')} onClick={toggleMobileMenu}></div>
                    ...
                    <div className='footer-menu-dropdown'>
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <button type='button' className='footer-menu-icon group'>
                                    ...
                                    <span className='app-switcher-label'>
                                        <div className='min-w-0'>
                                            <span className='block truncate text-sm font-bold dark:text-foreground'>
                                                {activePage?.name ?? 'All status pages'}
                                            </span>
                                            <span className='block truncate text-xs dark:text-muted-foreground'>
                                                {activePage?.domain ?? `${tenants.length} page${tenants.length === 1 ? '' : 's'}`}
                                            </span>
                                        </div>
                                        <svg
                            ...
                            <DropdownMenuContent align='end' side='top' className='w-60'>
                                    ...
                                    tenants.map(page => (
                                        <a
                                            ...
                                            <span className='ms-2 block min-w-0'>
                                                <span className='footer-menu-text truncate'>{page.name}</span>
                                                <span className='block truncate text-xs dark:text-muted-foreground'>
                                ...
                                <Link href={tenantsRoute()} className='tw-nav-footer-item group'>
                                    <span className='icon-[mdi--plus-circle-outline] size-4 shrink-0'></span>
                                    Add another status page
                                </Link>
                                <div className='item-group-divider'></div>
                                <button className='tw-nav-footer-item group flex justify-between' onClick={logout}>
                                    <span>Sign out</span>
